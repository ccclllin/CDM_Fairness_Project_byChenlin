...
model = HybridCDM(n_features=n_features, n_items=n_items).to(DEVICE)
model.load_state_dict(torch.load('output/hybrid_model.pth', map_location=DEVICE))
model.eval()
print("   ✓ 模型加载成功")
...
report = f"""
============================================================
实验组报告：混合模型（语义+敏感）公平性评估
============================================================

【模型配置】
- 输入特征: 语义向量 (100维) + 敏感属性 (4维) = 104维
- 模型: HybridCDM
...
with open('output/hybrid_eval_report.txt', 'w', encoding='utf-8') as f:
    f.write(report)

print("\n报告已保存: output/hybrid_eval_report.txt")
print("实验组 Step 4-3 完成！")